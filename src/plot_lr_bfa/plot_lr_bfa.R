...
infiles <- gsub("bfa_lr_fit_", "", infiles)
names(fits) <- infiles
...
saveRDS(all_hdi, file = "bfa_dco_bayes_hdi.rds")
saveRDS(all_map, file = "bfa_dco_bayes_map.rds")
saveRDS(all_pd, file = "bfa_dco_bayes_pd.rds")

orderly_artefact(
  files = c(
    "bfa_dco_bayes_hdi.rds",
    "bfa_dco_bayes_map.rds",
    "bfa_dco_bayes_pd.rds"
  ),
  description = "HDI, MAP, and PD for BFA  model fits"
)
