...
orderly_dependency(
  "process_drc_midline", "latest", files = c("drc_midline_dco.rds")
)

drc_midline_dco <- readRDS("drc_midline_dco.rds")
outfile_prefix <- "drc_midline_consult_length_by"

out <- summary(drc_midline_dco$consult_length_calc) |>
  round(2) |>
  tidy()
...
walk(cat_covariates, function(x) {
  ...
  small_df <- drc_midline_dco[!is.na(drc_midline_dco[[x]]), ]
  ...
  kable(as.data.frame(tab), format = "latex", booktabs = TRUE) |>
    writeLines(glue("drc_midline_dco_{x}.tex"))
 })

outfiles <- glue("drc_midline_dco_{cat_covariates}.tex")
